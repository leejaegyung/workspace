...
export function ProjectBoard() {
  ...
  const kanban: KanbanColumn[] = id ? (projectKanban[id] ?? []) : [];
  ...
  // Project members for AssigneePicker
  const allMembers = (project?.members ?? []).map((m) => ({
    name: m.name,
    initial: m.initial,
    color: m.color === 'primary' ? 'from-primary to-primary-dim'
         : m.color === 'secondary' ? 'from-secondary to-secondary-dim'
  ...
  async function handleCreateTask() {
    ...
    await addProjectKanbanTask(id, newTaskModal.colId, {
  ...
  async function handleSaveDetailField(field: Partial<KanbanTask>) {
    if (!detailModal.task || !id) return;
    await editProjectKanbanTask(id, detailModal.colId, detailModal.task.id, field);
    setDetailModal(prev => prev.task ? { ...prev, task: { ...prev.task, ...field } } : prev);
  }

  async function handleSaveEdit() {
    if (!editModal.task || !editTitle.trim() || !id) return;
    await editProjectKanbanTask(id, editModal.colId, editModal.task.id, { title: editTitle.trim(), desc: editDesc.trim() || undefined });
  ...
  async function handleMarkDone(colId: string, task: KanbanTask) {
    ...
    if (doneCol) {
      await deleteProjectKanbanTask(id, colId, task.id);
      await addProjectKanbanTask(id, doneCol.id, { ...task, isCompleted: true });
      toast(`🎉 "${task.title}" 완료!`, 'delight');
    }
  }

  async function handleAddColumn() {
    if (!colName.trim() || !id) return;
    await addProjectKanbanColumn(id, colName.trim());
  ...
  return (
    <div className="min-h-screen">
      ...
      <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-10">
        <div>
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-bold text-on-surface-variant hover:text-on-surface transition-colors mb-3"
          >
            <ArrowLeft className="w-3.5 h-3.5" /> {project.name}
          </button>
          <h1 className="text-4xl font-extrabold font-headline tracking-tight text-on-surface mb-2">칸반 보드</h1>
          <p className="text-on-surface-variant">{project.name} — 태스크 보드</p>
        </div>
      ...
      <div className="flex gap-8 overflow-x-auto pb-8 hide-scrollbar items-start">
        {kanban.map((col) => (
          <KanbanColView
            ...
            onDeleteColumn={() => {
              if (!id) return;
              deleteProjectKanbanColumn(id, col.id)
                .then(() => toast(`"${col.title}" 컬럼이 삭제되었습니다.`, 'info'))
      ...
      {detailModal.open && detailModal.task && (() => {
        ...
        const colorMap: Record<ColorType, { tag: string; bar: string; text: string }> = {
          primary:   { tag: 'border-primary bg-primary/10 text-primary',       bar: 'from-primary to-primary-dim',     text: 'text-primary'   },
          secondary: { tag: 'border-secondary bg-secondary/10 text-secondary', bar: 'from-secondary to-secondary-dim', text: 'text-secondary' },
          tertiary:  { tag: 'border-tertiary bg-tertiary/10 text-tertiary',     bar: 'from-tertiary to-tertiary-dim',   text: 'text-tertiary'  },
        };
              ...
              <div className="p-6">
                ...
                <div className="flex items-start justify-between gap-3 mb-5">
                  <div className="flex flex-wrap gap-2 flex-1">
                    <select value={detailTag} onChange={(e) => { setDetailTag(e.target.value); handleSaveDetailField({ tag: e.target.value }); }}
                    ...
                    <select value={detailPriority} onChange={(e) => { setDetailPriority(e.target.value); handleSaveDetailField({ priority: e.target.value || undefined }); }}
                    ...
                    {task.isCompleted && <span className="text-[0.65rem] font-bold tracking-wider uppercase px-2 py-0.5 rounded-full bg-tertiary/10 text-tertiary">완료</span>}
                  </div>
                  <button onClick={() => setDetailModal({ open: false, colId: '', task: null })}
                ...
                {/* Title */}
                {editingDetailTitle ? (
                  <input autoFocus value={detailTitle} onChange={(e) => setDetailTitle(e.target.value)}
                  ...
                  <h3 onClick={() => setEditingDetailTitle(true)}
                ...
                {/* Description */}
                {editingDetailDesc ? (
                  <textarea autoFocus value={detailDesc} onChange={(e) => setDetailDesc(e.target.value)}
                    ...
                    rows={3} className="w-full bg-white/5 rounded-xl px-3 py-2 text-sm border border-primary/30 focus:outline-none resize-none mb-4 placeholder:text-on-surface-variant/40"
                    placeholder="태스크 설명을 입력하세요..."
                  />
                ) : (
                  <p onClick={() => setEditingDetailDesc(true)}
                  ...
                  <div className="flex items-center gap-2 bg-white/[0.03] rounded-xl px-3 py-2.5">
                    ...
                    <div className="flex gap-1.5">
                      {(['primary', 'secondary', 'tertiary'] as ColorType[]).map(col => (
                        <button key={col} onClick={() => { setDetailColor(col); handleSaveDetailField({ color: col }); }}
                ...
                <div className="flex gap-2 pt-4 border-t border-white/5">
                  {!task.isCompleted && (
                    <button onClick={() => { handleMarkDone(detailModal.colId, task); setDetailModal({ open: false, colId: '', task: null }); }}
                  ...
                  <button onClick={() => { setDetailModal({ open: false, colId: '', task: null }); openEdit(detailModal.colId, task); }}
                  ...
                  <button onClick={() => { handleDeleteTask(detailModal.colId, task.id, task.title); setDetailModal({ open: false, colId: '', task: null }); }}
...
function AssigneePicker({ assignees, allMembers, onToggle }: {
  ...
  return (
    <div className="mb-5">
        ...
        <div className="relative" ref={ref}>
          ...
          {open && (
            <div className="absolute right-0 top-full mt-1 w-52 bg-surface-container-highest/95 backdrop-blur-xl rounded-xl border border-white/5 shadow-[0_8px_24px_rgba(0,0,0,0.5)] overflow-hidden z-50">
              {allMembers.length === 0 && (
                <p className="px-4 py-3 text-xs text-on-surface-variant">멤버가 없습니다.</p>
              )}
              {allMembers.map((m) => {
                const isAssigned = assignees.includes(m.name);
                return (
                  <button key={m.name} onClick={() => onToggle(m.name)}
                    className={cn('w-full px-3 py-2 flex items-center gap-3 text-sm transition-colors', isAssigned ? 'bg-primary/10' : 'hover:bg-white/5')}>
      ...
      {assignees.length === 0 ? (
        <button onClick={() => setOpen(true)} className="flex items-center gap-2 text-xs text-on-surface-variant hover:text-primary transition-colors">
          <div className="w-7 h-7 rounded-full border-2 border-dashed border-white/20 flex items-center justify-center"><Plus className="w-3 h-3" /></div>
          담당자를 지정하세요
        </button>
      ) : (
        <div className="flex items-center gap-2 flex-wrap">
          {assignees.map((name) => {
            ...
            return (
              <button key={name} onClick={() => onToggle(name)} title={`${name} 제거`}
                className="group flex items-center gap-1.5 bg-white/5 hover:bg-error/10 rounded-full pl-0.5 pr-2.5 py-0.5 transition-all">
                <div className={cn('w-6 h-6 rounded-full bg-gradient-to-br flex items-center justify-center text-[9px] font-bold text-surface', m.color)}>{m.initial}</div>
        ...
        <div className="flex -space-x-2">
          ...
          {(task.assignees ?? []).slice(0, 3).map((name) => (
            <div key={name} title={name}
              className="w-7 h-7 rounded-full bg-gradient-to-br from-primary to-secondary border-2 border-surface flex items-center justify-center text-[9px] font-bold text-surface">
              {name.charAt(0)}
            </div>
          ))}
